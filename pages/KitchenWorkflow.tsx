        ...
        <div className="h-[calc(100vh-6rem)] flex flex-col gap-6">
            ...
            <div className="flex justify-between items-center">
                <div className="flex gap-2">
                    <button 
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${view === 'list' ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                    >
            ...
            {view === 'list' && (
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 overflow-y-auto flex-1 transition-colors">
                    ...
                    {requests.length === 0 ? (
                        <p className="text-slate-500 dark:text-slate-400 text-center py-12">No requests found.</p>
                    ) : (
                        <div className="space-y-3">
                            {requests.map(req => (
                                <div key={req.id} className="flex justify-between items-center p-4 border border-slate-100 dark:border-slate-800 rounded-lg hover:border-emerald-300 dark:hover:border-emerald-700 transition-all bg-slate-50 dark:bg-slate-800/50">
                                    <div className="flex items-start gap-4">
                                        <div className={`p-3 rounded-full ${
                                            req.status === 'approved' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 
                                            req.status === 'rejected' ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400'
                                        }`}>
                                            {req.status === 'approved' ? <CheckCircle2 size={20} /> : req.status === 'rejected' ? <X size={20} /> : <Clock size={20} />}
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800 dark:text-white">{req.title}</h4>
                                    ...
                                    <button 
                                        ...
                                        className="px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg text-sm font-bold text-slate-600 dark:text-slate-300"
                                    >
            ...
            {view === 'create' && (
                <div className="max-w-2xl mx-auto w-full bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 overflow-y-auto transition-colors">
                    ...
                    <form onSubmit={handleSubmitRequest} className="space-y-6">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Workflow Title</label>
                            <input 
                                type="text" 
                                required
                                value={title}
                                ...
                                className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none"
                            />
                        </div>

                        <div>
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Description & Pain Points</label>
                            <textarea 
                                required
                                rows={4}
                                value={description}
                                ...
                                className="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none resize-none"
                            />
                        </div>

                        <div>
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Upload Media (Video/Images)</label>
                            <div 
                                onClick={() => fileInputRef.current?.click()}
                                className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-emerald-400 dark:hover:border-emerald-500 transition-colors"
                            >
                                <Upload size={32} className="text-slate-400 dark:text-slate-500 mb-2" />
                                <p className="text-sm font-bold text-slate-600 dark:text-slate-300">Click to upload files</p>
                                <p className="text-xs text-slate-400 dark:text-slate-500">Max 50MB per file</p>
                                <input 
                                    type="file" 
                                    ref={fileInputRef} 
                                    className="hidden" 
                                ...
                                <div className="mt-4 space-y-2">
                                    {files.map((file, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg">
                                            ...
                                            <button type="button" onClick={() => removeFile(idx)} className="p-1 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500 dark:text-red-400 rounded">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    ))}
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
                            <button type="button" onClick={() => setView('list')} className="px-6 py-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white font-bold">Cancel</button>
                            <button 
                                type="submit" 
            ...
            {view === 'detail' && selectedRequest && (
                <div className="flex flex-col lg:flex-row gap-6 h-full overflow-hidden">
                    ...
                    <div className="w-full lg:w-1/3 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col overflow-hidden transition-colors">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                            <button onClick={() => setView('list')} className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1 mb-4 hover:text-slate-800 dark:hover:text-white">
                                <ArrowLeft size={12} /> Back
                            </button>
                            <h2 className="text-xl font-bold text-slate-800 dark:text-white">{selectedRequest.title}</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Requested by {selectedRequest.userName}</p>
                        </div>
                        <div className="p-6 overflow-y-auto flex-1">
                            <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Description</h4>
                            <p className="text-sm text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-100 dark:border-slate-700 mb-6">
                                {selectedRequest.description}
                            </p>

                            <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Uploaded Media</h4>
                            <div className="space-y-3">
                                {selectedRequest.mediaFiles.map((file, i) => (
                                    <div key={i} className="flex items-center gap-3 p-3 border border-slate-200 dark:border-slate-700 rounded-lg">
                                        ...
                                        <button className="text-xs bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600">
                                            View
                                        </button>
                                    </div>
                                ))}
                    ...
                    <div className="flex-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col overflow-hidden transition-colors">
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
                            <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <GitMerge size={20} className="text-emerald-600 dark:text-emerald-400" /> Workflow Design
                            </h3>
                            <div className="flex items-center gap-2">
                                <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${
                                    selectedRequest.status === 'approved' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400'
                                }`}>
                            ...
                            {isAdmin && selectedRequest.status === 'pending' ? (
                                <>
                                    ...
                                    <textarea 
                                        ...
                                        className="flex-1 w-full p-4 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-mono bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none resize-none"
                                    ...
                                    <div className="mt-4 flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                                        <button onClick={rejectRequest} className="px-4 py-2 text-red-600 dark:text-red-400 font-bold hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-sm">Reject</button>
                                        <button 
                                ...
                                <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700">
                                    {selectedRequest.adminResponse ? (
                                        <div className="prose prose-sm max-w-none text-slate-700 dark:text-slate-300">
                                            {/* Simple markdown rendering simulation */}
                                            {selectedRequest.adminResponse.split('\n').map((line, i) => (
                                                <p key={i} className={`mb-2 ${line.startsWith('#') ? 'font-bold text-slate-900 dark:text-white text-lg mt-4' : ''}`}>
                                                    {line.replace(/#/g, '')}
                                                </p>
                                            ))}
                                        </div>
